...
def build_user_prompt(
    ...
    for index, item in enumerate(chunks, start=1):
        source_blocks.append(
            f"[S{index}] {item.source.title}\n"
            f"Publisher: {item.source.publisher}\n"
            f"Evidence tier: {item.source.evidence_tier}\n"
    ...
    return (
        f"<care_mode>{care_mode}</care_mode>\n"
        f"<minimum_urgency>{assessment.urgency.value}</minimum_urgency>\n"
        f"<safety_flags>{','.join(assessment.flags) or 'none'}</safety_flags>\n"
